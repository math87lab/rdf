...
def to_rdf(w, df):
    for i in range(len(df)):
        ...
        if name or name_kana or address or direction or latitude or longitude or phone_number or extention_number:
            w.write(subject + ' <http://imi.go.jp/ns/core/2#設置地点> ' + '_:空白ノード7 .\n')
            if name or name_kana:
                w.write('_:空白ノード7' + ' <http://imi.go.jp/ns/core/2#名称> ' + '_:空白ノード8 .\n')
                if name:
                    w.write('_:空白ノード8' + ' <http://imi.go.jp/ns/core/2#表記> ' + f'"{name}" .\n')
                if name_kana:
                    w.write('_:空白ノード8' + ' <http://imi.go.jp/ns/core/2#カナ表記> ' + f'"{name_kana}" .\n')
            if address or direction:
                w.write('_:空白ノード7' + ' <http://imi.go.jp/ns/core/2#住所> ' + '_:空白ノード9 .\n')
                if address:
                    w.write('_:空白ノード9' + ' <http://imi.go.jp/ns/core/2#表記> ' + f'"{address}" .\n')
                if direction:
                    w.write('_:空白ノード9' + ' <http://imi.go.jp/ns/core/2#方書> ' + f'"{direction}" .\n')
            if latitude or longitude:
                w.write('_:空白ノード7' + ' <http://imi.go.jp/ns/core/2#地理座標> ' + '_:空白ノード10 .\n')
                if latitude:
            ...
            if phone_number or extention_number:
                w.write('_:空白ノード7' + ' <http://imi.go.jp/ns/core/2#連絡先> ' + '_:空白ノード11 .\n')
                if phone_number:
        ...
        if url:
            w.write(subject + ' <http://imi.go.jp/ns/core/2#参照> ' + '_:空白ノード17 .\n'
                    + '_:空白ノード17' + ' <http://imi.go.jp/ns/core/2#参照先> ' + f'"{url}"^^<http://www.w3.org/2001/XMLSchema#anyURI> .\n')
