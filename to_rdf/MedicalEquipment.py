...
def to_rdf(w, df):
    for i in range(len(df)):
        ...
        if name or name_kana:
            w.write(subject + ' <http://imi.go.jp/ns/core/2#名称> ' + '_:空白ノード9 .\n')
            if name:
                w.write('_:空白ノード9' + ' <http://imi.go.jp/ns/core/2#表記> ' + f'"{name}" .\n')
            if name_kana:
                w.write('_:空白ノード9' + ' <http://imi.go.jp/ns/core/2#カナ表記> ' + f'"{name_kana}" .\n')
        if variety:
        ...
        if phone_number or extension_number or fax_number or corporate_number or corporate_name or code:
            w.write(subject + ' <http://imi.go.jp/ns/core/2#連絡先> ' + '_:空白ノード12 .\n')
            if phone_number:
                w.write('_:空白ノード12' + ' <http://imi.go.jp/ns/core/2#電話番号> ' + f'"{phone_number}" .\n')
            if extension_number:
            ...
            if corporate_name:
                w.write('_:空白ノード12' + ' <http://imi.go.jp/ns/core/2#名称> ' + '_:空白ノード13 .\n'
                        + '_:空白ノード13' + ' <http://imi.go.jp/ns/core/2#表記> ' + f'"{corporate_name}" .\n')
            if corporate_number or code:
                w.write('_:空白ノード12' + ' <http://imi.go.jp/ns/core/2#ID> ' + '_:空白ノード14 .\n')
                if corporate_number:
                    w.write('_:空白ノード14' + ' <http://imi.go.jp/ns/core/2#識別値> ' + f'"{corporate_number}" .\n')
                if code:
                    w.write('_:空白ノード14' + ' <http://imi.go.jp/ns/core/2#識別値> ' + f'"{code}".\n')
        if day:
        ...
        if start_time or end_time or day_detail:
            ...
            if start_time:
                w.write('_:空白ノード17' + ' <http://imi.go.jp/ns/core/2#開始時間> ' + f'"{day_detail}" .\n')
            if end_time:
                w.write('_:空白ノード17' + ' <http://imi.go.jp/ns/core/2#終了時間> ' + f'"{day_detail}" .\n')
            if day_detail:
                w.write('_:空白ノード17' + ' <http://imi.go.jp/ns/core/2#説明> ' + f'"{day_detail}" .\n')

        if support_overtime or remark:
            if support_overtime:
        ...
        if bed_num:
            w.write(subject + ' <http://imi.go.jp/ns/core/2#収容人数> ' + '_:空白ノード18 .\n'
                    + '_:空白ノード18' + ' <http://imi.go.jp/ns/core/2#数値> ' + f'"{bed_num}" .\n')
